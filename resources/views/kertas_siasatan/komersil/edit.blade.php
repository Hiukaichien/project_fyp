<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('kertas_siasatan.update', ['paperType' => 'KomersilPaper', 'id' => $paper->id]) }}" class="space-y-6 bg-white p-6 shadow rounded">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 p-3 bg-gray-50 rounded border">
                    ...
                    <div><span class="font-semibold">Pegawai Siasatan:</span> {{ $paper->io_aio }}</div>
                    <div><span class="font-semibold">Seksyen:</span> {{ $paper->seksyen }}</div>
                    <div><span class="font-semibold">Tarikh KS Dibuka:</span> {{ optional($paper->tarikh_ks_dibuka)->format('d/m/Y') ?? '-' }}</div>
                </div>
                <hr>
