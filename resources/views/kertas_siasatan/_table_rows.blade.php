{{-- resources/views/kertas_siasatan/_table_rows.blade.php --}}
@forelse ($kertasSiasatans as $index => $ks)
    <tr>
        ...
        <td class="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900">{{ $ks->no_ks }}</td>
        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{{ optional($ks->tarikh_ks)->format('d/m/Y') ?? '-' }}</td>
        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{{ $ks->no_report ?? '-' }}</td>
        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{{ $ks->pegawai_penyiasat ?? '-' }}</td>
...
{{-- Include pagination links within the partial if you want them updated too --}}
{{-- Note: This simple approach might cause layout shifts or duplicate pagination controls --}}
{{-- A more robust solution might involve separate JS handling for pagination --}}
{{-- <tr id="pagination-row"> <td colspan="8"> <div class="mt-4"> {{ $kertasSiasatans->links() }} </div> </td> </tr> --}}